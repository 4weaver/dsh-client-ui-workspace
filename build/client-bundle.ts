/**
 * FORK-LOCAL BUILD PRESET — copied from the upstream monorepo, not imported.
 *
 * Source:   packages/client/tsdown.client.ts
 * Upstream: https://github.com/deepseek-ai/deepseek-harness
 * Base tag: dsh-v0.1.2-rc.1   (base commit a66e470204)
 * Copied:   2026-09-10, from the fork branch's `forktree/dev` checkout.
 *
 * WHY THIS FILE EXISTS
 * The upstream preset lives inside the monorepo coordinate system: it derives
 * REPOSITORY_ROOT as `../..`, globs the workspace manifests under
 * `packages/<group>/<package>/` to find the manifest it builds, and imports
 * three siblings that are NOT available to a standalone consumer (see the
 * resolved-imports block below). The fork is published as a single-package
 * repository, so it carries this copy to build its own client bundle without
 * ever reaching the monorepo.
 *
 * REBASE OBLIGATION — this file is a FORK COPY, not a dependency.
 * On every upstream rc bump, re-diff this file against
 * `packages/client/tsdown.client.ts` at the new tag. Search for the marker
 * `FORK PLAN A` to find every deliberate divergence; there are exact-count
 * markers on each. A silent divergence here is the main risk of this design.
 *
 * ---- upstream file documentation follows, verbatim ----
 *
...
// ==== FORK PLAN A: the three sibling imports, resolved (all three copied) ====
// Upstream had, in this order:
...
// All three resolve only inside the monorepo source tree. Each is now a
// fork-local copy carrying its own provenance header:
//
// 1. PLATFORM_MODULES / PRELOADED_CLIENT_EXTERNALS -> ./platform.ts
//    The published `@deepseek-ai/dsh-client-web@0.1.2-rc.1` does export both
//    (lib/index.js:283), but that module is NOT loadable by a config loader:
//    it carries module-level `.css` imports and eagerly imports its whole peer
//    graph, and npm publishes its @deepseek-ai deps at versions that do not
//    match this fork's — so importing it would drag in wrong-version packages
//    whose runtime identity must match the host's single instances. Copied.
// 2. optionalStringArray -> ./optional-string-array.ts (defined but not exported).
// 3. clientBuildEnvironmentDefines -> ./client-build-environment.ts (not published).
import { PLATFORM_MODULES, PRELOADED_CLIENT_EXTERNALS } from './platform.ts'
import { optionalStringArray } from './optional-string-array.ts'
...
// ==== FORK PLAN A: repository coordinates, re-derived for a flat repo ====
// Upstream: `const REPOSITORY_ROOT = fileURLToPath(new URL('../..', import.meta.url))`
// — correct for `packages/client/tsdown.client.ts`, i.e. two levels above the
// monorepo root. This preset sits at `<repo>/build/client-bundle.ts`, so `'..'`
// is the repository root directly. Literal rather than `import.meta.url`
// because tsdown may transpile this module, and a resolve relative to THIS
// file is what the flat layout means by "repository root".
const REPOSITORY_ROOT = resolvePath(dirname(fileURLToPath(import.meta.url)), '..')

/**
 * Rebase a physical lib-relative source onto the URL the browser uses to fetch it.
 *
 * Upstream mapped `<root>/packages/<group>/<package>/src/x.ts` onto
 * `../../../packages/<group>/<package>/src/x.ts`, because the dev server
 * serves the map from `/plugins/<scoped-package>/client.js.map` and the browser
 * resolves relative entries against that two-segment-deep path.
 *
 * In a flat repo there is no `packages/` tree: a source resolves to
 * `<root>/src/...`, so the repo-relative form is already the `/src/...` URL
 * the devtools server exposes (the fork's dev mount serves the package root).
 * Emitting it relative reproduces the upstream depth convention exactly —
 * `../../../src/...` — while the `packages/` special case disappears with the
 * tree it described.
 */
...
/**
 * A physical path re-rooted onto the repository, for consumers that hash their
 * input: `/<repo>/src/a.module.css` becomes `/src/a.module.css`.
 *
 * FORK PLAN A. lightningcss derives CSS-modules class hashes from the
 * `filename` it is handed, so passing an absolute path made the emitted class
 * names a function of the checkout's LOCATION rather than of its contents —
 * the same stylesheet hashed to VxvziG at /tmp/planA-build, H26cja at
 * /tmp/planA-build2, and YvHw6q from the monorepo. Because the leading `/`
 * makes the repository root the filesystem root, this is always the clean
 * `/src/...` form (never `../src/...`), so the hash is stable for any build
 * directory — the property a fixed-output derivation depends on.
 * @param physicalPath - an absolute path inside this repository.
 * @returns the path as if the repository root were `/`.
 */
function repositoryAssetPath(physicalPath: string): string {
  return `/${relative(REPOSITORY_ROOT, physicalPath).split(sep).join('/')}`
}
...
export function clientBundle(
  ...
  // FORK PLAN A: the manifest locator is per-build state. Every manifest read
  // happens on a resolveId, i.e. strictly inside this returned build function,
  // so assigning here is correctly scoped and cannot leak across builds.
  locateManifest = options.locateManifest ?? monorepoManifest
  manifestCache.clear()
  return ({ env }) => {
...
interface ClientBundleOptions {
  ...
  /**
   * How this build finds the bundled package's manifest. Defaults to the
   * upstream monorepo workspace glob; a single-package repository passes the
   * flat-layout locator. FORK PLAN A — the only option this fork adds.
   */
  readonly locateManifest?: ManifestLocator
}
 ...
 */
// ==== FORK PLAN A: the hard coupling — manifest location ====
// Upstream globs `packages/*/*/package.json` under the repository root. A
// single-package repository has no `packages/` tree, and the glob cannot be
// re-pointed at one: `**/` would match tsdown's own `node_modules/**` (found
// FIRST at the tree root), and `*/` would not match the root `package.json`
// at all. The flat layout is therefore a different locator, not a different
// glob — so the locator is injectable per build and the default stays the
// upstream monorepo one.

/**
 * How a build locates the manifest of the package it is bundling.
 * @param id - package name, as spelled at the preset call site.
 * @returns the parsed manifest, or `undefined` when this locator cannot find it.
 */
type ManifestLocator = (id: string) => WorkspaceManifest | undefined

/**
 * UPSTREAM BEHAVIOUR, UNCHANGED: locate one workspace package by name. Located
 * by package name rather than by cwd, because tsdown evaluates every package
 ...
 * @param id - package name, as spelled at the preset call site.
 * @returns the parsed manifest, or `undefined` when no workspace package declares that name.
 */
const monorepoManifest: ManifestLocator = (id) => {
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: REPOSITORY_ROOT })) {
    ...
    // ponytail: build-only alias — the fork mounts under a local name, so a
    // config that names it borrows this package's manifest (same file, checked
    // into this branch). Delete with the fork.
    if (manifest.name !== id && !(manifest.name === '@deepseek-ai/dsh-client-ui-workspace' && id === 'dsh-client-ui-forkspace')) continue
    return manifest
  }
  return undefined
}
...
/**
 * THE FLAT-LAYOUT LOCATOR. The bundled package's manifest is this repository's
 * own `package.json`; there is exactly one package here, so the name check is
 * the whole lookup. `process.cwd()` is tsdown's documented config-directory
 * contract for a single-package build and IS the repository root for this
 * layout, so no cwd-independent path derivation is needed — and none is
 * possible, since tsdown does not hand a config its own file path.
 *
 * The upstream name-alias hack is GONE: this repo's manifest already carries
 * the fork's id, so nothing has to be borrowed.
 * @param id - package name, as spelled at the preset call site.
 * @returns the parsed manifest, or `undefined` when it declares another name.
 */
export const localManifest: ManifestLocator = (id) => {
  const path = resolvePath(process.cwd(), 'package.json')
  if (!existsSync(path)) return undefined
  const manifest = JSON.parse(readFileSync(path, 'utf8')) as WorkspaceManifest
  return manifest.name === id ? manifest : undefined
}

let locateManifest: ManifestLocator = monorepoManifest

function workspaceManifest(id: string): WorkspaceManifest {
  const cached = manifestCache.get(id)
  if (cached !== undefined) return cached
  const manifest = locateManifest(id)
  if (manifest === undefined) {
    throw new Error(locateManifest === localManifest
      ? `tsdown: ${process.cwd()}/package.json does not declare the name ${id}`
  ...
  return {
    ...
    plugins: [{
      ...
      async load(virtualId: string) {
        ...
        // FORK PLAN A: the virtual id carries the repo-relative path (see this
        // plugin's resolveId), so it can no longer be used as a filesystem path.
        // Re-anchor onto REPOSITORY_ROOT for every real IO, and keep the relative
        // form for lightningcss's `filename` below.
        const repositoryPath = virtualId.slice(CSS_VIRTUAL_PREFIX.length, -CSS_VIRTUAL_SUFFIX.length)
        const fileId = resolvePath(REPOSITORY_ROOT, repositoryPath.slice(1))
        // The virtual id otherwise hides the physical stylesheet from Rolldown's watch graph.
        this.addWatchFile(fileId)
        const source = await readFile(fileId)
        const { code, exports: cssExports } = transform({
          // FORK PLAN A: give lightningcss a REPO-ROOT-ANCHORED path.
          // It derives the CSS-modules `[hash]` from this string, so an
          // absolute path makes the emitted class names depend on where the
          // checkout lives — the same stylesheet produced VxvziG under
          // /tmp/planA-build, H26cja under /tmp/planA-build2, and YvHw6q from
          // the monorepo. Calling the result `filename` makes the ROOT the
          // virtual directory, which yields exactly `/src/...` (no `..` to
          // escape); the hash then depends only on the file's repo-relative
          // identity, so the bundle is reproducible across directories —
          // which is what a fixed-output Nix derivation needs. Upstream never
          // hits this because every build shares one repository root.
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        return INLINE_CSS_VIRTUAL_PREFIX + repositoryAssetPath(abs) + CSS_VIRTUAL_SUFFIX
      },
      async load(virtualId: string) {
        if (!virtualId.startsWith(INLINE_CSS_VIRTUAL_PREFIX)) return null
        // FORK PLAN A: the virtual id is repo-relative (see resolveId above), so
        // re-anchor it for IO and use the relative form for `filename` — these
        // paths reach the emitted region comments, which must not carry an
        // absolute build directory.
        const repositoryPath = virtualId.slice(INLINE_CSS_VIRTUAL_PREFIX.length, -CSS_VIRTUAL_SUFFIX.length)
        const fileId = resolvePath(REPOSITORY_ROOT, repositoryPath.slice(1))
        this.addWatchFile(fileId)
        const source = await readFile(fileId)
        const { code } = transform({ filename: repositoryPath, code: source, minify: true })
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const abs = importer !== undefined ? sourceAssetPath(source, importer) : source
        return GLOBAL_CSS_VIRTUAL_PREFIX + repositoryAssetPath(abs) + CSS_VIRTUAL_SUFFIX
      },
      async load(virtualId: string) {
        ...
        // paths reach the emitted region comments, which must not carry an
        // absolute build directory.
        const repositoryPath = virtualId.slice(GLOBAL_CSS_VIRTUAL_PREFIX.length, -CSS_VIRTUAL_SUFFIX.length)
        const fileId = resolvePath(REPOSITORY_ROOT, repositoryPath.slice(1))
        this.addWatchFile(fileId)
        const source = await readFile(fileId)
        const { code } = transform({ filename: repositoryPath, code: source, minify: true })
