/**
 * FORK-LOCAL COPY of the upstream monorepo script `scripts/client-build-environment.ts`.
 *
 * Source:   scripts/client-build-environment.ts  (393 lines, copied WHOLESALE)
 * Upstream: https://github.com/deepseek-ai/deepseek-harness
 * Base tag: dsh-v0.1.2-rc.1   (base commit a66e470204)
 *
 * WHY THE WHOLE FILE AND NOT JUST `clientBuildEnvironmentDefines`
 * Only `clientBuildEnvironmentDefines` (upstream :263) is consumed by the
 * preset, but it is NOT free-standing: it calls the module-private
 * `clientBuildEnvironment` (:173), which reads `Office[...]`. A partial copy
 * would therefore not be self-contained and would silently rot. The file has
 * ZERO npm dependencies — its imports are only node:crypto, node:child_process,
 * node:fs and node:path — so the whole thing copies cleanly, exactly as
 * `scripts/` (which has no package.json and is published as no package) can
 * never be imported from outside the monorepo.
 *
 * REBASE OBLIGATION: re-diff against upstream on every rc bump. No deliberate
 * divergence — this file is byte-identical to upstream below this header.
 */
